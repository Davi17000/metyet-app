...
.cp-bind-view { margin-top: 8px; width: 100%; justify-content: center; padding: 3px 6px; font-size: 11px; }
.cp-bind-x { margin-top: 5px; }
          ...
          {rows.map((p, i) => (
            <div key={p.card.id} className="cv-row">
              ...
              <div className="cv-main">
                <div className="cv-t">
                  {cardTitle(p.card)}
                  {/* No physical copy exists here — identity research only. */}
...
function CardCopyActions({ ctx, card: c, copy, compact, showCert = true }) {
  const { say } = ctx;
  if (!c) return null;
    ...
    <div className={"ccopy" + (compact ? " compact" : "")}>
      ...
      {cert && (
        <button className="btn sm ccopy-b" title={"Copy PSA certification number — " + cert}
          ...
          <Icon n="copy" s={12} />{!compact && <span>PSA Cert #</span>}
        </button>
      )}
    </div>
  );
...
function CounterFields({ amt, setAmt, reference, pctLabel, pctAria, amtAria }) {
  ...
  const usable = percentageOf(1, reference) != null;   // is there a reference at all?
...
function MarketDecision({ tc, by, theirHeading, myHeading, refLabel, party, onAccept, onPropose }) {
  ...
  const share = shareText(mine, theirs);
  ...
  return (
    <div className="pn row">
      ...
      <div className="pn-side">
        ...
        {mine != null && !opening && (
          <div className="pn-pct">Last sent {money(mine)}{share ? " · " + share + " of " + refLabel : ""}</div>
        )}
        <CounterFields amt={amt} setAmt={setAmt} reference={theirs} pctLabel={"% of " + refLabel}
          amtAria="Market value in dollars"
          pctAria={"Market value as a percentage of " + refLabel} />
        <button className="btn sm pn-send" disabled={!validAmount(amt)}
...
function MarketWaiting({ tc, by, who, refLabel, party }) {
  const mine = by === "tp" ? tc.tpMarket : tc.collectorMarket;
  const theirs = by === "tp" ? tc.collectorMarket : tc.tpMarket;
  if (mine == null) return null;
  const share = shareText(mine, theirs);
  return (
    <div className="pn row wait">
      <div className="pn-side">
        ...
        <div className="pn-pct">{share ? share + " of " + refLabel + " previous value" : "sent"}</div>
      </div>
      <div className="pn-wait">Waiting on {who}</div>
    </div>
  );
  ...
  return (
    <div className="ws-top">
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <CardCopyActions ctx={ctx} card={c} copy={boundInv} />
        {others > 1 && (
...
function BinderCard({ ctx, cc }) {
  ...
  return (
    <div className={"cp-bind" + (cc.tpInterest ? " on" : "")}>
      ...
      {/* Inspect the actual copy before deciding. Informational only — it carries no
          negotiation, no stage controls, and never touches tpInterest. */}
    ...
    <div className="ovl" onClick={() => setDrawer(null)}>
      <div className="drawer" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mb">
          ...
          <div className="kv"><span className="k">Collector value</span><span className="v"><Money v={cc.market} /></span></div>
